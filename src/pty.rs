...
//     let mut pt = PseudoTerminal::new(size, cmd, Some("password".to_string()));
//     terminal.clear().unwrap();
//     let mut show_cursor = true;

//     loop {
//         if pt.terminate.load(std::sync::atomic::Ordering::Relaxed) {
//             break;
//         }

//         terminal
//             .draw(|frame| {
//                 let parser = pt.parser.read().unwrap();
//                 let screen = parser.screen();
//                 let block = Block::default().style(Style::default().bg(Color::Black));
...
//                 }
//                 Event::Key(key) => {
//                     pt.handle_key_event(&key).await;
//                 }
//                 Event::Resize(cols, rows) => {
//                     size.cols = cols;
//                     size.rows = rows;

//                     pt.parser.write().unwrap().set_size(rows, cols);
//                     pt.master
//                         .resize(PtySize {
//                             rows: rows,
//                             cols: cols,
...
//     let buffer = pt.buffer.lock().unwrap();
//     if buffer.contains("Last login") {
//         if let Some(start) = buffer.rfind("password: ") {
//             let end = buffer[start + 10..].find('\n').unwrap();
//             passwd = Some(
